...
import static com.demo.mybluetoothdemo.utils.bleutils.BleConnectUtil.mBluetoothGattCharacteristicNotify;
...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 10:
                    //接收到数据，显示在界面上
                    dialog.dismiss();
                    Log.e(TAG, "收到数据，显示在界面上:" + msg.obj.toString());
    ...
    private void scanBle() {
        ...
        bleConnectUtil.bluetoothIsAble(new MyBleCallBack() {
            ...
            public void callbleBack(BluetoothDevice device) {
                Log.e(TAG, "--->搜索到的蓝牙名字：" + device.getName());
                if (device == null) {
                    return;
                }
    ...
    @OnClick({R.id.btn_scan, R.id.btn_send, R.id.btn_write,R.id.btn_notify, R.id.btn_disconnect})
...
//                    Log.e(TAG, "获取数据的方法点击啦------");
//                    boolean b = bleConnectUtil.readData(mBluetoothGattCharacteristic);
//                    Log.e(TAG, "b-----" + b);
                    bleConnectUtil.mCharacteristic();
                } else {
                    Toast.makeText(MainActivity.this, "请连接蓝牙", Toast.LENGTH_SHORT).show();
                }
...
//                    boolean b = bleConnectUtil.readData(mBluetoothGattCharacteristic);
//                    Log.e(TAG, "b-----" + b);
                    bleConnectUtil.readCharacteristic();
        ...
        switch (eventMsg.getMsg()) {
            case Constants.BLE_CONNECTION_FINISH_MSG:
                ...
                if (bleConnectUtil.isConnected()) {
                    ...
                    Log.e(TAG, "wocao aowcaowocaowocao ");
                } else {
...
//            message.what = 100;
//            handler.sendMessage(message);
//            bleConnectUtil.readData(mBluetoothGattCharacteristic);
//            mBluetoothGatt
            //    bleConnectUtil.readData(mBluetoothGattCharacteristic);
        }
...
//                showDialog(title);
            ...
            if (currentSendAllOrder.length() <= 40) {
                ...
                bleConnectUtil.writeCharacteristic(sData);
                //mBluetoothGattCharacteristic.writeCharacteristic
//           BluetoothGattDescriptor descriptor = mBluetoothGattCharacteristic.getDescriptor(UUID.fromString("6326edac-53ec-4e94-916d-6d9702abb4de"));
...
//                mBluetoothGattCharacteristic.addDescriptor(descriptor);
            //  mBluetoothGattCharacteristic.setValue(sData);
               // isSuccess[0] = bleConnectUtil.readData(mBluetoothGattCharacteristic);
                // isSuccess[0] = bleConnectUtil.sendData(mBluetoothGattCharacteristic);
                // isSuccess[0] = bleConnectUtil.sendCmd(mBluetoothGattCharacteristic,2000);
                //              bleConnectUtil.readData(mBluetoothGattCharacteristicNotify);
             //   isSuccess[0] = bleConnectUtil.sendData(mBluetoothGattCharacteristic);
            } else {
                for (int i = 0; i < currentSendAllOrder.length(); i = i + 40) {
                    ...
                    Log.e("TAG", "shortOrder[0]2：" + shortOrder[0]);
                    sData = CheckUtils.hex2byte(shortOrder[0]);
                    mBluetoothGattCharacteristic.setValue(sData);
                    //isSuccess[0]  = bleConnectUtil.sendCmd(mBluetoothGattCharacteristic,500);
                    isSuccess[0] = bleConnectUtil.sendData(mBluetoothGattCharacteristic);
                }
            }
